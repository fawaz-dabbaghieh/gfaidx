...
void configure_get_region_parser(argparse::ArgumentParser& parser) {
    ...
    // Deprecated: kept only so existing scripts/docs that pass
    // --all_haplotypes keep working. It is equivalent to --mode all and is
    // rejected if it disagrees with an explicitly passed --mode.
...
int run_get_region(const argparse::ArgumentParser& program) {
    try {
        ...
        // Both non-BFS modes share the exact-coordinate selection pipeline
        // below; --reference_only-style behavior only skips the part of it
        // that resolves every OTHER haplotype's span.
        const bool all_haplotypes = (mode == "all" || mode == "reference");
        const bool reference_only = (mode == "reference");
        ...
        if (reference_only) {
            // reference_only implies all_haplotypes by construction above, so
            // this only needs to check mode == "reference" itself. Skip
            // query_path_haplotype_nodes entirely: exact_reference_path_runs
            // was already resolved above from the coordinate index (or the
            // on-the-fly fallback), cheaply and independently of any other
            // haplotype. Building the node set from just those runs avoids the
            // postings scan over every reference-covered node, which is what
            // makes --mode all expensive on graphs with many haplotypes.
            if (exact_reference_path_runs.empty()) {
                throw std::runtime_error(
                    "--mode reference found no exact reference path run for this "
                    "interval; the queried reference/sequence may not have an "
                    "indexed P/W record covering it");
            }
            const auto reference_only_node_ranks =
                select_reference_only_nodes(path_index, exact_reference_path_runs);
            std::cout << get_time() << ": Reference-only selection resolved "
                      << reference_only_node_ranks.size() << " nodes from "
                      << exact_reference_path_runs.size()
                      << " reference path run(s); no other haplotypes were scanned"
                      << std::endl;
            return chunk::extract_subgraph_from_node_ranks(
                options,
                reference_only_node_ranks,
