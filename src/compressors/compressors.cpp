//
// Created by Fawaz Dabbaghie on 18/12/2025.
//
...
std::string rle_encode(const std::string_view s_line, const std::size_t seq_start, const std::size_t seq_end) {
...
// --------------------
// Varint (uint32_t), 7 payload bits per byte, most-significant bit continuation
// --------------------
...
// --------------------
// Example parser: reads tokens like "123+" or "123-" separated by commas/whitespace
// Encodes each as packed uint32 varint into `out`.
// --------------------
inline void encode_path_string_ids_u32(const std::string& path_string,
    ...
    auto skip_seps = [&](size_t& pos) {
        while (pos < path_string.size() &&
               (path_string[pos] == ',' || std::isspace(static_cast<unsigned char>(path_string[pos]))))
            ++pos;
    };

    while (true) {
        skip_seps(i);
        if (i >= path_string.size()) break;

        // Parse string_id up to '+' or '-'
        const size_t start = i;
        while (i < path_string.size() && path_string[i] != '+' && path_string[i] != '-') ++i;
        if (i >= path_string.size()) throw std::runtime_error("expected '+' or '-' after node id");

        std::string node_id = path_string.substr(start, i - start);
        if (node_id.empty()) throw std::runtime_error("empty node id");

        const bool is_rev = (path_string[i] == '-');
        ++i; // consume '+' or '-'

        // Look up integer id
        auto it = id_map.find(node_id);
        if (it == id_map.end()) {
